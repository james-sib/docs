...
<Note>
The `turn` spans group each exchange **inside the single conversation trace**. They are not LangSmith [thread](/langsmith/threads) turns: a voice conversation is kept as one trace, not split into a separate trace per turn, so interruptions and inter-turn timing are preserved.
</Note>
