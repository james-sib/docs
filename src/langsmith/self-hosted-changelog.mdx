...
- **LLM auth proxy deployments without a public ingress**: If the LLM auth proxy is only reachable through internal Kubernetes networking, set the required SSRF environment variables before using it. See [Deploy without a public ingress](/langsmith/llm-auth-proxy-self-hosted#deploy-without-a-public-ingress).
