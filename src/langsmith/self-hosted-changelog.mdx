...
- Fixed column width issue in automations table to enhance UI readability.
- Improved run stats query performance by removing unnecessary joins.
- Updated frontend with new evaluator template packs for enhanced evaluator management.
- Improved playground UI by gating rendering on feature config loading state.
- Added support for agent cost breakdown and user cost table in the fleet management interface.
- Added new endpoints for querying and managing fleet-admin permissions.
- Improved evaluator sorting and filtering options for better user experience.
- Introduced new API endpoints to handle evaluator creation with specified sort options.
- Enhanced security with URL allowlist enforcement for JWT injection in polly configuration.
- Added support for evaluating created and updated dates in evaluator management.
- Improved UI responsiveness and user experience for various frontend components.
- Improved API performance by concurrent data writing and optimized latency checks.
- Added organization members endpoint to manage workspace roles and permissions.
- Enhanced security by adding forced mTLS support in database migrations.
- Improved tracing project management with default groupBy setting on project selection.
- Added support for new tools and integrations in agent builder configuration.
- Fixed UI bugs and enhanced feedback functionality in sandbox and evaluator interfaces.
- Improved layout and styling for various frontend elements in the tracing project interface.
- Enhanced option settings in agent builder and new integrations setup wizard.
- Enabled mobile-friendly login with PWA support for improved accessibility and user experience.
- Improved session sync performance for faster data processing in background operations.
- Added new endpoints for minting self-hosted licenses with JWT authentication.
- Expanded messaging and profiler features with additional audit logging and metadata attributes.
...
- Fixed truncation issue in 'Enabled' column of the automations table.
- Bumped icon package and added icon variants in the frontend.
- Updated details view header for improved UI.
- Improved performance by removing dead joins from session stats queries.
- Added back button on full page trace view for navigation.
- Enhanced evaluator sorting and filtering options in the UI.
- Made MCP server filter dropdown scrollable for better navigability.
- Fixed file creation bug that cleared active interrupt state in fleet.
- Added URL allowlist enforcement for JWT injections for improved security.
- Added support for async agents in fleet to enhance operations.
- Improved layout and functionality of sandbox proxy configurations.
- Added evaluator feedback key and resource cell filtering for better usability.
- Introduced several organizational and permission features, including fleet-admin:read permission for enhanced access control.
- Enhanced evaluator table with new search and filtering options based on distinct fields like 'created by'.
- Introduced trace-based tooltip enhancements on usage pages.
- Fixed OAuth provider icon and naming mismatch issues.
- Enhanced UI by allowing the detachment of run rules from the resources tab.
- Enhanced performance by applying cache timeout updates.
- Updated evaluator usability with edit warnings and UI consistency improvements.
- Improved evaluator reuse functionality with UI refinements and bug fixes.
- Improved agent cloning and run rule creation flows with enhanced error handling.
- Enhanced tool usage and UI functionalities including tool usage tab and load tracing project names.
- Added OAuth-related improvements for authentication and session backends.
- Fixed Bug prior to addition of external endpoints for sensitive data access.
- Improved sandbox and self-hosted deployment configurations by adding various features like ability for instance admin to restart backfill jobs.
- Enhanced sandbox-related SSH support and environmental setup capabilities.
- Enabled detailed logging and build improvements for CI/CD pipelines.
- Improved tracing, rule, and run functionalities for clearer and smoother operation management.
...
- Improved evaluator details view header styling.
- Enhanced performance by removing an unnecessary join from session stats queries.
- Fixed an issue where the MCP server filter dropdowns were not scrollable.
- Updated evaluator templates packs for better usability.
- Introduced new API endpoints for run statistics insights and dashboards.
- Added a new authentication feature to enforce URL allowlist for JWT injection.
- Enabled sorting by 'Created At' and 'Updated At' in evaluators table.
- Added new API endpoints for ABAC (attribute-based access control) to create sessions.
- Improved evaluator details UX, including showing 'Feedback Key' in columns dropdown.
- Enabled evaluator reuse and added a feedback banner for evaluator launch.
- Added support for model GLM5 and Minimax 2.5.
- Added the ability to detach run rules from resources in the UI.
- Expanded support for Anthropic clients in tool call parsing.
- Enhanced evaluator actions handling when creating issue board issues.
- Redesigned memory tool rendering and approval UX for a smoother user experience.
- Added support for GitHub OAuth user authentication flow and installation sync.
- Introduced new API endpoints to manage audit logs and datasets.
- Improved usage table design with tool icons and CSV export capability.
- Enhanced models support with additions to the model pricing maps such as Claude Opus 4.7.
- Fixed multiple UI bugs and improved frontend performance and rendering.
- Improved SSO login and implemented mTLS support for migrations.
...
- Fixed decoding of OTLP/JSON traceId/spanId to use hex, improving integration with tracing tools.
