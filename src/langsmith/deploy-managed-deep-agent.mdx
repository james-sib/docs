...
Each `tools[].name` must match the tool name **the MCP server itself exposes** (the name returned by the server's `tools/list` response), not the workspace's MCP server display name. For example, Tavily's MCP server exposes `tavily_search` with an underscore — even if you registered the server under a different name like `tavily`, the agent's `tools[].name` must still be `tavily_search`. If the name doesn't match, the runtime silently drops the tool from the model's bound tool set and the agent never gets to call it.

The `interrupt_config` map lets you require human approval for selected tools. Each key is `{mcp_server_url}::{tool_name}` (separator is two colons); trailing slashes on the URL are stripped before matching. Additional `::`-separated parts after the tool name (such as the MCP server display name shown in the example above) are accepted but ignored when matching. Set the value to `true` to require human approval before the tool runs, or `false` to allow it without interrupt. See [Resume from a human-in-the-loop interrupt](#resume-from-a-human-in-the-loop-interrupt) for the resume payload the run expects when an interrupt fires.
...
When `interrupt_config` flags a tool with `true`, the run pauses before that tool executes. The pause is delivered as part of the SSE stream — an `__interrupt__` field appears inside a `values` (or `updates`) event, then the stream closes:
  ...
  "__interrupt__": [
    {
      "value": {
        "action_requests": [
          {
            "name": "tavily_search",
            "args": { "query": "agent memory tradeoffs", "max_results": 5 },
...
To act on the pause, post a follow-up run on the **same thread** with a `command.resume` payload. The run picks back up from the interrupted node and either runs the tool, blocks it, or substitutes a synthetic tool reply — depending on the decision.
...
<Tabs>
  <Tab title="Python (httpx)">
    ...
    resume_payload = {
        "agent_id": agent_id,
        # The API requires a non-empty `messages` array on every run call.
...
<Warning>
The resume value must be the `HITLResponse` dict `{"decisions": [...]}` — **not** a bare list of decisions. The middleware processes the resume by accessing `decisions = interrupt(request)["decisions"]`, so a top-level list crashes the run with `TypeError: list indices must be integers or slices, not str`.
</Warning>

Send exactly one decision per `action_request`, in the same order. Each `type` must appear in the matching `review_configs[i].allowed_decisions` — otherwise the run raises `ValueError`.

| Decision | Body | Effect |
| --- | --- | --- |
| Approve | `{"type": "approve"}` | Runs the tool with the proposed args. |
| Edit | `{"type": "edit", "edited_action": {"name": "...", "args": {...}}}` | Runs the tool with modified name/args (the original `tool_call.id` is preserved). |
| Reject | `{"type": "reject", "message": "..."}` | Blocks the tool. The model receives an error `ToolMessage` containing `message` (or a default rejection notice). |
| Respond | `{"type": "respond", "message": "..."}` | Skips the tool. The model receives a success `ToolMessage` whose content is `message`, as if the tool had returned it. |

If multiple tool calls are interrupted in the same step (parallel tool calls), the `action_requests` array contains one entry per call and you must send a parallel `decisions` array of the same length and order.

### Cancel an interrupted thread

`POST /v1/deepagents/threads/{thread_id}/resolve-interrupt` is **not** an approve shortcut — it advances the upstream LangGraph runtime to the `__end__` node, terminating the paused run without invoking the pending tool. Use it only when you want to abandon the paused work and free the thread. For any actual approve / edit / reject / respond decision, send a `command.resume` to `/runs/stream` as shown above.
