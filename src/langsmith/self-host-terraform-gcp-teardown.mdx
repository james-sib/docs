...
2. **Without Terraform state** — manual cleanup via gcloud CLI when state is lost.

Both follow the same reverse-dependency order. Pick the section that matches your situation.

<Warning>
Teardown permanently deletes the Cloud SQL instance, GCS bucket contents, and Secret Manager secrets. Export any data you need to retain before proceeding.
</Warning>
