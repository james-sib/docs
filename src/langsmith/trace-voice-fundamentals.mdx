...
<Note>
Keep the whole conversation in one trace. Do not split it into a separate trace per turn, the thread-level "turn" model common for text chat: a voice conversation's exchanges overlap and interrupt, and splitting them loses the timing and barge-in information between them. Some framework integrations add a `turn` span that groups each exchange *inside* the conversation trace; that grouping run is distinct from a thread-level turn.
</Note>
