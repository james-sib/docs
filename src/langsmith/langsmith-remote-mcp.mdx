...
<Note>
**LangSmith Cloud (SaaS) only.** The Remote MCP is currently available on all LangSmith Cloud regions (`api.smith.langchain.com`, `eu.api.smith.langchain.com`, and `aws.api.smith.langchain.com`). [Self-hosted LangSmith](/langsmith/self-hosted) deployments should continue to use the [standalone LangSmith MCP Server](/langsmith/langsmith-mcp-server).
</Note>
...
The server discovers the rest of its OAuth metadata via [RFC 8414](https://datatracker.ietf.org/doc/html/rfc8414) at `/.well-known/oauth-authorization-server` on the same host, so a compliant MCP client only needs the URL for its region.
...
<Note>
**OpenAI Codex CLI** does not currently work with the LangSmith Remote MCP. Codex omits the [RFC 8707](https://datatracker.ietf.org/doc/html/rfc8707) `resource` parameter required by the [MCP authorization spec](https://modelcontextprotocol.io/specification/draft/basic/authorization) during the OAuth flow, so login appears to succeed but the issued token is not bound to the LangSmith MCP and `initialize` fails with an auth-required error. Tracked upstream in [openai/codex#20729](https://github.com/openai/codex/issues/20729) (token exchange) and [openai/codex#13891](https://github.com/openai/codex/issues/13891) (authorize request). Until those land, use the [LangSmith CLI](/langsmith/langsmith-cli) from Codex—it covers the same projects, traces, runs, datasets, experiments, and threads surface as the MCP server, with native OAuth login.
</Note>
