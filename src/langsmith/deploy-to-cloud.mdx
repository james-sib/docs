...
When enabled for a deployment, opening a PR against the deployment's branch can trigger the creation of a new, temporarily linked deployment called a **preview deployment**. In **Every PR** mode, any PR against the branch triggers a preview, while in **Label only** mode, a preview is created only when the configured label is added to the PR.

On creation, the preview deployment builds the latest commit from the PR's source branch for its first revision. Subsequent commits to that branch automatically trigger new revisions.

<Note>
Preview builds are only available for deployments created through the GitHub integration.
</Note>
...
1. Check **Enable preview builds**.
1. Leave the trigger mode as **Every PR**, or switch to **Label only** and configure a trigger label.
1. Set the **Idle TTL** and **Max concurrent previews** as desired.
1. Select **Save**.

**Idle TTL:** If the time elapsed since the preview's last revision (its most recent commit) exceeds this configured value, the preview deployment will be deleted.

**Max concurrent previews:** Caps the number of preview deployments that can be active concurrently for a parent deployment.

### Secrets

A preview deployment inherits the secrets of its parent deployment, but you can manually override them after creation.

<Note>
Changes to the parent deployment's secrets after a preview deployment is created do not automatically propagate to the preview deployment.
</Note>

### Deletion

Preview deployments are tied to their parent deployment. Deleting the parent deployment triggers a cascaded deletion of its preview deployments. Preview deployments are also automatically torn down when their configured TTL expires, and can be manually deleted at any time like any other deployment.
