...
<Tip>
**Prefer infrastructure as code?** [Deploy with Terraform](/langsmith/self-host-terraform) bundles cluster provisioning, secrets wiring, and the Helm release for AWS, Azure, GCP, and OpenShift into one workflow. The page below covers the Helm-only path against any conformant cluster you already manage.
</Tip>
