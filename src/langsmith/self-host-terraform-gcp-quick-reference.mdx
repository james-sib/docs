...
For the full deployment walkthrough, see the [GCP quickstart](/langsmith/self-host-terraform-gcp-quickstart).
...
| 3 | LangSmith Deployment (host-backend, listener, operator) | `make apply && make init-values && make deploy` |
...
Each pass builds on the previous. Verify pods are healthy before enabling the next.
...
- `config.deployment.enabled: true` is required for Pass 3. Setting only the URL without `enabled: true` silently skips `listener` and `operator`.
...
```bash
# 1. Remove LangSmith Deployment resources (if Pass 3 was enabled)
...
For the full procedure, see the [GCP teardown guide](/langsmith/self-host-terraform-gcp-teardown).
