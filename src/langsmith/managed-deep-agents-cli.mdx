...
For the fastest end-to-end path, see the [quickstart](/langsmith/managed-deep-agents-quickstart). For workflow guides, see [Connect tools](/langsmith/managed-deep-agents-mcp) and [Deploy an agent](/langsmith/managed-deep-agents-deploy).
...
Bare `deepagents` invocations do not start an interactive REPL. Install and run [Deep Agents Code](/oss/deepagents/code/overview) for interactive coding sessions.
...
The legacy `model_id` key is still accepted in local subagent files, but new projects should use `model`.
