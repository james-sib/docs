...
When ABAC policies are active, resources are access-controlled based on their tags. To make sure a resource is protected from the moment it is created — without any window where it is untagged — you can supply tags directly in the creation request using the `tag_value_ids` parameter.
