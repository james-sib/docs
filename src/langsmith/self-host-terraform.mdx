...
<CardGroup cols={2}>
  ...
  <Card title="Azure (AKS)" icon="brand-windows" href="/langsmith/self-host-terraform-azure-quickstart">
    Provision AKS, Azure Database for PostgreSQL, Azure Managed Redis, Blob Storage, and Key Vault. Status: GA.
  </Card>
  <Card title="GCP (GKE)" icon="brand-google" href="/langsmith/self-host-terraform-gcp-quickstart">
    Provision GKE, Cloud SQL, Memorystore, GCS, and Workload Identity. Status: GA.
  </Card>
  <Card title="OpenShift (OCP)" icon="brand-redhat" href="/langsmith/self-host-terraform-ocp-quickstart">
    Configure namespaces, RBAC, operators, and storage classes on an existing OCP or ROSA cluster. Status: Preview.
  </Card>
</CardGroup>

All four provider paths are documented. AWS, Azure, and GCP are GA; the OpenShift path is in preview while the Terraform module matures.
...
<Steps>
  ...
  <Step title="Pass 2 — LangSmith">
    Helm installs the LangSmith chart against the cluster from Pass 1. Secrets sync from the cloud-native store through [External Secrets Operator](https://external-secrets.io/), so no credentials live in `tfvars` or git.
  </Step>
</Steps>

Azure offers optional passes for [LangSmith Deployment](/langsmith/deploy-self-hosted-full-platform), Agent Builder, and Insights. AWS and GCP cover the same optional add-ons in a single follow-up step.
