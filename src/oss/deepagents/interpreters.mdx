...
A library is a directory with two files:
...
**`LIBRARY.md`** declares metadata in YAML frontmatter and provides documentation in the markdown body:

| Field | Required | Description |
| --- | --- | --- |
| `name` | Yes | Module name used in `import` statements (e.g., `"swarm"`) |
| `description` | Yes | One-line summary shown to the agent in its system prompt |
| `ptcTools` | No | Host-side tools the library needs at runtime (e.g., `[write_file, read_file]`) |
...
**`index.ts`** (or `index.js`) is the entrypoint that runs inside the QuickJS sandbox. TypeScript type annotations are stripped automatically. The agent imports from this module with `import { ... } from "my-library"`.

```typescript
export function normalize(orders) {
  return orders.map((o) => ({
    ...o,
...
export function score(orders, criteria) {
  return orders
    .map((o) => ({ ...o, total: criteria.reduce((s, c) => s + Number(o[c.key] ?? 0) * c.weight, 0) }))
...
The agent doesn't see full library documentation upfront. When libraries are loaded, the agent's system prompt gets a one-line summary of each (the `description` from frontmatter). The full LIBRARY.md body with API details, examples, and usage notes is available on demand:

```javascript
const docs = await tools.readFile({
  file_path: "/libraries/my-library/LIBRARY.md",
});
```

This keeps the agent's initial context small. For simple tasks the agent can work from function signatures alone. For complex tasks it pulls up the docs.

### PTC

The `ptcTools` field in LIBRARY.md declares which host-side [programmatic tool call](#programmatic-tool-calling) bridges the library needs. Inside library code, they're available on the `tools` global:
...
Use `loadLibrary` to load a custom library from a directory, then pass it to `createCodeInterpreterMiddleware`:

```typescript
import { createDeepAgent } from "deepagents";
import { createCodeInterpreterMiddleware, loadLibrary } from "@langchain/quickjs";

const myLib = await loadLibrary("./libraries/my-library");
...
`@langchain/quickjs` ships with pre-built libraries that you can add to any agent. See [Pre-built libraries](/oss/deepagents/pre-built-libraries) for setup and API reference.

| Library | Description |
| --- | --- |
| [Swarm](/oss/deepagents/pre-built-libraries#swarm) | Parallel task fan-out with a table-based data model. Three-function API: `create`, `run`, `rows`. |
...
Call `evaluate(candidates, criteria)` to run a full comparison pipeline.
````
...
```typescript
import { createCodeInterpreterMiddleware, swarm, loadLibrary } from "@langchain/quickjs";

const swarmLib = swarm({ defaultModel, subagents: [...] });
const evaluatorLib = await loadLibrary("./libraries/evaluator");
