...
`@langchain/quickjs` ships with prebuilt libraries that you can add to any agent. See [Interpreter libraries](/oss/deepagents/interpreter-libraries) for setup and API reference.

| Library | Description |
| --- | --- |
| [Swarm](/oss/deepagents/interpreter-libraries#swarm) | Parallel task fan-out with a table-based data model. Three-function API: `create`, `run`, `rows`. |
