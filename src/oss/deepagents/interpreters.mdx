...
:::python
<Tip>
    Using Deep Agents Code (`dcode`)? Enable the same middleware as a `js_eval` tool with the `--interpreter` flag. See [Interpreter](/oss/deepagents/code/interpreter) for the CLI flow, the `safe` / `all` PTC presets, and the local-mode constraint.
</Tip>
:::
