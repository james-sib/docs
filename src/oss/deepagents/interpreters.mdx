...
Interpreter libraries let you package reusable capabilities that agents can import from interpreter code. You define the source, tools, and usage instructions at configuration time. At runtime, the agent writes `import { ... } from "my-library"` and gets access to whatever the library exposes.

Libraries can import other libraries, so you can compose higher-level abstractions on top of lower-level ones.
...
**Source code** is JavaScript or TypeScript that runs inside the QuickJS sandbox. The agent imports from the library by name (e.g., `import { ... } from "my-library"`).

**Instructions** are injected into the agent's system prompt so it knows how to use the library. Write these as markdown with a quick start example and API reference.
...
const myLib: InterpreterLibrary = {
  ...
  instructions: fs.readFileSync("./libraries/my-library/INSTRUCTIONS.md", "utf-8"),
  files: new Map([
    ["utils.ts", fs.readFileSync("./libraries/my-library/utils.ts", "utf-8")],
    ["schemas.ts", fs.readFileSync("./libraries/my-library/schemas.ts", "utf-8")],
  ]),
};
...
Load both libraries together. Subagents referenced by the library (`researcher`) are registered on `createDeepAgent`:
...
const agent = createDeepAgent({
  model,
  subagents: [
    { name: "researcher", description: "Researches a topic in depth", systemPrompt: "..." },
  ],
  middleware: [createCodeInterpreterMiddleware({ libraries: [swarmLib, evaluatorLib] })],
});
```
