...
Explicit `-a`/`--agent` always overrides both, and `-r`/`--resume` bypasses both so the thread's original agent is restored. See [Command reference](/oss/deepagents/code/overview#command-reference) for related flags.
...
Because Deep Agents Code imports the `class_path` class at startup, the package that defines it must be importable from the same Python environment that runs `dcode`. This is usually **not** your project virtual environment. If you installed `dcode` with the install script, use `dcode --install` with the `--package` flag to add custom model packages to the `dcode` environment:

```bash
dcode --install my-company-models --package
```

In a session, run `/install my-company-models --package` and confirm the prompt. Both install the package alongside `dcode`. If the package is missing or cannot be imported, Deep Agents Code skips the provider and its models do not appear in `/model`.

For example, imagine your company has an internal package, `my-company-models`, that exposes a LangChain-compatible chat model at `my_company_models.deepagents:CompanyChatModel`. After installing the package into the `dcode` environment, configure it like this:

```toml
[models.providers.company]
class_path = "my_company_models.deepagents:CompanyChatModel"
models = ["company-chat-prod"]
api_key_env = "COMPANY_MODEL_API_KEY"

[models.providers.company.params]
region = "us-east-1"
timeout = 30
```

Then start `dcode` and select the model with `/model company:company-chat-prod`, or pass it on the command line:

```bash
dcode --model company:company-chat-prod
```

When you switch to `company:company-chat-prod` (via `/model` or `--model`), the model name (`company-chat-prod`) is passed as the `model` kwarg:

```python
CompanyChatModel(model="company-chat-prod", region="us-east-1", timeout=30)
```
