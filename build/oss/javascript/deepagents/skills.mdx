...
- **Read-only skills**: Use [filesystem permissions](/oss/javascript/deepagents/permissions) to deny write operations under skill paths.
...
- **Human-in-the-loop**: Use [`interrupt_on`](/oss/javascript/deepagents/human-in-the-loop) to require approval before the agent executes write operations, adding a review step without fully blocking writes.
