...
For multi-tenancy, authentication, memory and sandbox scoping, guardrails, and frontend integration beyond this CLI walkthrough, see [Going to production](/oss/javascript/deepagents/going-to-production).
...
| **`AGENTS.md`** | Required. [Memory](/oss/javascript/deepagents/memory) for the agent. Provides persistent context (project conventions, instructions, preferences) that is always loaded at startup. Read-only at runtime. |
| **`skills/`** | _Optional_. Directory of [skill](/oss/javascript/deepagents/skills) definitions that provide specialized workflows and domain knowledge. Each subdirectory should contain a `SKILL.md` file. Read-only at runtime. Dotfiles are skipped when uploading. |
...
| Field | Type | Description |
| --- | --- | --- |
| `provider` | `string` | _Optional_. Sandbox provider. Supported values: `"none"`, `"daytona"`, `"modal"`, `"runloop"`, `"langsmith"` (private beta). The relevant partner package is automatically included in the deployment. See [sandbox integrations](/oss/javascript/integrations/sandboxes) for provider details. Defaults to `"none"`. When omitted or set to `provider = "none"`, the sandbox is disabled. |
...
Place a `.env` file alongside `deepagents.toml` with your API keys. For sandbox code that needs to call external APIs without leaking credentials, prefer the [sandbox auth proxy](/oss/javascript/deepagents/going-to-production#managing-secrets) over baking secrets into `.env`.
...
User memory gives each user their own writable `AGENTS.md` that persists across conversations. This is the CLI's built-in equivalent of the user-scoped memory pattern described in [Going to production: Memory](/oss/javascript/deepagents/going-to-production#memory). To enable it, create a `user/` directory at your project root:
...
If your agent needs custom tools, custom middleware (rate limiting, retries, PII redaction), or production patterns the CLI doesn't expose, deploy directly via [LangSmith Deployments](/langsmith/deployment) and follow [Going to production](/oss/javascript/deepagents/going-to-production) for the underlying patterns ([guardrails](/oss/javascript/deepagents/going-to-production#guardrails), [durability](/oss/javascript/deepagents/going-to-production#durability), [scoped memory](/oss/javascript/deepagents/going-to-production#scoping)).
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss/deepagents/deploy/overview.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
