...
If your LLM stage is an in-process [LangGraph or LangChain](/oss/python/langgraph/overview) agent, pass `configure_pipecat(llm_span_kind="chain")` so its model and tool runs nest correctly inside Pipecat's `llm` span.
