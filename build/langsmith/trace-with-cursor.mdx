...
In a local clone of [`langsmith-cursor-plugins`](https://github.com/langchain-ai/langsmith-cursor-plugins), install the hooks for all Cursor projects (writes `~/.cursor/hooks.json`):
...
<Note>
The repository includes a precompiled JavaScript bundle in `bundle/`, so the hooks run without `pnpm install` or `pnpm build`. Rebuild only after modifying the TypeScript source.
</Note>
